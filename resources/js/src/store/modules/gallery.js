...
const mutations = {
    ...
    SET_UPLOAD_CHILD(state, payload) {
        state.uploadChild = payload
    },
    ...
    SET_FORM_PHOTOS(state, payload) {
        state.photos = payload
    },
    SET_FORM_INDEX(state, payload) {
        state.formIndex = payload
    },
    SET_FORM_INDEXS(state, payload) {
        state.formItem.index = payload
    },
...
const actions = {
    Index({ commit, state }) {
        ...
        return new Promise((resolve) => {
            ...
            $axios
                .get(`/gallery?page=${page}&perpage=${itemsPerPage}`)
    ...
    Update({ dispatch, commit, state }) {
        return new Promise((resolve) => {
            $axios
                .patch(`/gallery/${state.formIndex}`, state.formItem)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        setTimeout(() => {
                            localStorage.setItem('token', null)
    ...
    UpdateChild({ dispatch, commit, state },payload) {
        return new Promise((resolve) => {
                let formData = new FormData();
                formData.append('child', true);
                formData.append('_method', payload._method);
                for (let i = 1; i <= payload.img.length; i++) {
                    formData.append('foto_'+i, payload.img[parseInt(i)-1]);
                }
            $axios
                .post(`/gallery/${state.formIndex}`, formData)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        setTimeout(() => {
                            localStorage.setItem('token', null)
