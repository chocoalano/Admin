...
const actions = {
    Index({ commit, state }) {
        ...
        return new Promise((resolve) => {
            ...
            $axios
                .get(`/config/page?page=${page}&perpage=${itemsPerPage}`)
    ...
    Show({ commit, state }) {
        return new Promise((resolve) => {
            $axios
                .get(`/config/page/${state.formIndex}`)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        setTimeout(() => {
                            localStorage.setItem('token', null)
    ...
    Update({ dispatch, commit, state }) {
        return new Promise((resolve) => {
            $axios
                .patch(`/config/page/${state.formIndex}`, state.formItem)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        setTimeout(() => {
                            localStorage.setItem('token', null)
    ...
    Store({ dispatch, commit, state }) {
        return new Promise((resolve) => {
            $axios
                .post(`/config/page`, state.formItem)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        localStorage.setItem('token', null)
                        commit('SET_TOKEN', null, { root: true })
