...
const actions = {
    Index({ commit, state }) {
        ...
        return new Promise((resolve) => {
            ...
            $axios
                .get(`/config/medsos?page=${page}&perpage=${itemsPerPage}`)
    ...
    Show({ commit, state }) {
        return new Promise((resolve) => {
            $axios
                .get(`/config/medsos/${state.formIndex}`)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        setTimeout(() => {
                            localStorage.setItem('token', null)
    ...
    Update({ dispatch, commit, state }) {
        return new Promise((resolve) => {
            $axios
                .patch(`/config/medsos/${state.formIndex}`, state.formItem)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        setTimeout(() => {
                            localStorage.setItem('token', null)
    ...
    Store({ dispatch, commit, state }) {
        return new Promise((resolve) => {
            $axios
                .post(`/config/medsos`, state.formItem)
                .then(function (response) {
                    if (response.status === 200 && response.data.status === "Token is Expired") {
                        localStorage.setItem('token', null)
                        commit('SET_TOKEN', null, { root: true })
