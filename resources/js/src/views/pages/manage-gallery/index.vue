...
export default {
  ...
  computed: {
    ...
    options: {
      get: function () {
        return this.$store.state.gallery.options
      },
      set: function (value) {
        this.$store.commit('gallery/SET_OPTIONS', value)
      },
    },
    loading: {
      get: function () {
        return this.$store.state.gallery.loading
      },
      set: function (value) {
        this.$store.commit('gallery/SET_LOADING', value)
      },
    },

    dialog: {
      get: function () {
        return this.$store.state.gallery.dialog
      },
      set: function (value) {
        this.$store.commit('gallery/SET_DIALOG', value)
      },
    },
    dialogDelete: {
      get: function () {
        return this.$store.state.gallery.dialogDelete
      },
      set: function (value) {
        this.$store.commit('gallery/SET_DIALOG_DELETE', value)
      },
    },
    formIndex: {
      get: function () {
  ...
  watch: {
    ...
    formItem(e) {
      this.$store.commit('gallery/SET_FORM_INDEXS', e)
    },
  },
  mounted() {
    this.getDataFromApi()
  ...
  methods: {
    ...mapActions('gallery', ['Index', 'Show', 'Update', 'Store', 'Delete']),
    ...
    editItem(item) {
      this.$store.commit('gallery/SET_FORM_INDEX', item)
      this.Show().then(res => {
        const form = {
          index: true,
          group_name: res.data.data.group_name,
          page_display: res.data.data.page_display,
          status: res.data.data.status,
        }
        this.$store.commit('gallery/SET_FORM', form)
        this.$store.commit('gallery/SET_FORM_PHOTOS', res.data.data.img)
      })
      this.dialog = true
    },
    ...
    close() {
      this.$store.commit('gallery/SET_FORM_INDEX', -1)
      this.$store.commit('gallery/CLEAR_FORM')
